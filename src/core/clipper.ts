...
export class VideoClipper {
  static validateTimeRange(
    ...
    if (startSeconds < 0 || endSeconds > duration) {
      throw new Error(
        `Clip range out of bounds: ${clip.start} to ${clip.end}, with duration ${duration}s`
      );
    }
  }
  /**
  ...
  static async extractClips(
    ...
    for (let i = 0; i < clips.length; i++) {
      ...
      c.log(`Processing clip ${i + 1}/${clips.length} (${strategy})...`);
      ...
      results.push(result);
      c.log(`✓ Created ${clip.output} in ${(clipDuration / 1000).toFixed(1)}s`);
    }
